...
Mat LPFilter;         //低通滤波器
Mat FFT_LP;          //原图的快速傅立叶矩阵FFT与低通滤波器LPFilter相乘结果
Mat FFT_LP_amlp;      //FFT_LP的傅里叶谱
int lpType;           //低通滤波器类型
const int MAX_LPTYPE = 2;
Mat flpSpectrum;    //傅立叶谱的灰度级
Mat result;			//低通滤波后的原图结果
string windowName = "低通滤波";

//得到低通滤波后的滤波器 -只与原图中心坐标、频率截断和n有关，与原图的具体像素无关
	...
	if (type == BLP_FILTER)
	{
		for (int r = 0; r < rows; r++)
		{
			for (int c = 0; c < cols; c++)
			{
				...
				float dist = sqrt(pow(r - center.y, 2) + pow(c - center.x, 2));
				lpFilter.at<float>(r, c) = 1.0/(1.0+pow(dist/radius,2.0*n));
			}
		}
	}
	...
	if (type == GLP_FILTER)
	{
		for (int r = 0; r < rows; r++)
		{
			for (int c = 0; c < cols; c++)
			{
				...
				lpFilter.at<float>(r, c) = exp(-pow(dist,2)/(2*pow(radius,2.0)));
			}
		}
	}
...
{
	//7.根据中心位置，频率截断和n得到低通滤波器
	LPFilter = createLPFilter(FFT.size(), maxLoc, radius, lpType, 2);

	//8.低通滤波器与原图的快速傅立叶变换矩阵相乘得到低通滤波下的傅立叶变换
	...
	//傅立叶谱/ 频谱图并没有在低通滤波中有实际的变换操作，
	//只是用来展示一些效果和找到频谱图的中心位置以构建低通滤波器
	//真正操作的其实就是傅立叶变换矩阵

	//额外2.相乘后的低通傅里叶矩阵的频谱图/傅里叶谱的灰度级展示
	FFT_LP_amlp = amplitudeSpectrum(FFT_LP);
	FFT_LP_amlp = grayAmplitude(FFT_LP_amlp);
	imshow("低通傅里叶谱灰度级展示", FFT_LP_amlp);

	//9.低通滤波下的傅立叶矩阵进行傅里叶逆变换
...
int main()
{
	...
	//低通滤波即过滤掉图像中高频部分，保留低频部分，类似于平滑操作，最后图像趋于平滑
	...
	createTrackbar("低通类型", windowName, &lpType, MAX_LPTYPE, callback_lpFilter);
